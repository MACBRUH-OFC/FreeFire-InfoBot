from flask import Flask, request, Response
from telegram import Update
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes
import json, io, requests
from PIL import Image

BOT_TOKEN = "7606678509:AAEpJmTxn-SHvgeNY2VZH5RN0j7Tr_i_MgQ"
AUTHORIZED_GROUP_IDS = [-1002453016338, -1002699301861]
AUTHORIZED_USER_IDS = [1745313119]

app = Flask(__name__)
application = Application.builder().token(BOT_TOKEN).build()

ITEM_DATA_URL = "https://raw.githubusercontent.com/MACBRUH-OFC/FreeFire-Resources/refs/heads/main/data/itemData.json"
IMAGE_BASE_URL = "https://raw.githubusercontent.com/MACBRUH-OFC/FreeFire-Resources/main/live/IconCDN/android/"

def load_item_data():
    try:
        response = requests.get(ITEM_DATA_URL)
        return json.loads(response.text)
    except:
        return []

def process_image(image_url):
    try:
        response = requests.get(image_url, timeout=10)
        img = Image.open(io.BytesIO(response.content))
        if img.mode in ('RGBA', 'LA'):
            bg = Image.new('RGB', img.size, 'black')
            bg.paste(img, mask=img.split()[-1])
            img = bg
        out = io.BytesIO()
        img.save(out, format='PNG')
        out.seek(0)
        return out
    except:
        return None

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Send item ID like this:\n\nid 1000")

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cid = update.effective_chat.id
    uid = update.effective_user.id
    if not (cid in AUTHORIZED_GROUP_IDS or (cid == uid and uid in AUTHORIZED_USER_IDS)):
        return await update.message.reply_text("Access denied.\nJoin @FreeFire_MacbruhUpdates")

    msg = update.message.text.strip()
    if msg.startswith("id ") and msg[3:].isdigit():
        item_id = int(msg[3:])
        item_data = load_item_data()
        item = next((i for i in item_data if i["itemID"] == item_id), None)
        if item:
            image_url = f"{IMAGE_BASE_URL}{item_id}.png"
            caption = f"""▫️ ITEM DETAILS ▫️
➖➖➖➖➖➖➖
ID: {item['itemID']}
Name: {item['description']}
Desc: {item.get('description2', item['description'])}
Icon: {item['icon']}
➖➖➖➖➖➖➖"""

            img_bytes = process_image(image_url)
            if img_bytes:
                await update.message.reply_photo(photo=img_bytes, caption=caption)
            else:
                await update.message.reply_text(caption + "\n⚠️ Image not available")
        else:
            await update.message.reply_text("❌ ID not found")

application.add_handler(CommandHandler("start", start))
application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))

@app.route("/api/index", methods=["POST"])
def index():
    update = Update.de_json(request.get_json(force=True), application.bot)
    application.update_queue.put_nowait(update)
    return Response("ok", status=200)
